...
export default function SettingsTaxRates() {
  ...
  useEffect(() => {
    ...
    apiRequest(`/api/taxrates${workspace}`, { token: currentUser.token })
      ...
      .catch((error) => toast.error(error.message || 'Failed to load tax rates'))
  ...
  async function save() {
    ...
    try {
      const res = await apiRequest(
        editing
          ? `/api/taxrates/${encodeURIComponent(editing.id)}${workspace}`
          : `/api/taxrates${workspace}`,
        {
  ...
  async function removeById(id) {
    ...
    const workspace = `?workspace=${encodeURIComponent(tenantId)}`
    try {
      await apiRequest(`/api/taxrates/${encodeURIComponent(id)}${workspace}`, {
        method: 'DELETE',
        token: currentUser.token,
      })
  ...
  return (
    <div>
      ...
      {loading ? <p className="text-sm text-gray-500">Loading tax rates...</p> : null}
      {!loading && items.length === 0 ? <EmptyState title="No tax rates" /> : null}
      ...
      <ConfirmDialog
        ...
        description="This may fail if products still use this tax rate."
        confirmLabel="Delete"
        danger
        onConfirm={() => {
